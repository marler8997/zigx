...
fn parseDisplay(s: []const u8) !x11.ParsedDisplay {
    return (x11.Display{ .string = s }).parse();
}
...
fn testParseDisplay(
    ...
    const parsed = try display.parse();
...
test "parseDisplay" {
    ...
    try testing.expectError(error.BadDisplayNumber, parseDisplay(":1x11."));
    try testing.expectError(error.BadDisplayNumber, parseDisplay(":1x11.10"));
...
fn takeXauthEntry(reader: *x11.AuthReader) std.Io.Reader.Error!?Auth {
