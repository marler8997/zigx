...
const TEST_RECEIVED_CONNECT_SETUP_BUFFER align(4) = [_]u8{
...
test "parseDisplay" {
    ...
    try testing.expectError(error.HasMultipleProtocols, x.parseDisplay("tcp//"));
    try testing.expectError(error.NoDisplayNumber, x.parseDisplay("0"));
    try testing.expectError(error.NoDisplayNumber, x.parseDisplay("unix/"));
    try testing.expectError(error.NoDisplayNumber, x.parseDisplay("inet/1"));
    try testing.expectError(error.NoDisplayNumber, x.parseDisplay(":"));

    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":a"));
    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":0a"));
    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":0a."));
    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":0a.0"));
    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":1x"));
    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":1x."));
    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":1x.10"));
    try testing.expectError(error.BadDisplayNumber, x.parseDisplay(":70000"));

    try testing.expectError(error.BadScreenNumber, x.parseDisplay(":1.x"));
    try testing.expectError(error.BadScreenNumber, x.parseDisplay(":1.0x"));
    ...
    if (builtin.os.tag == .windows) {
        ...
        try testing.expectError(error.NoDisplayNumber, x.parseDisplay("w32"));
    }
}
...
test "VisualType.findMatchingVisualType" {
    ...
    const connect_setup_stub = x.ConnectSetup{
        .buf = try allocator.allocWithOptions(u8, TEST_RECEIVED_CONNECT_SETUP_BUFFER.len, if (zig_atleast_15) .@"4" else 4, null),
    };
    defer connect_setup_stub.deinit(allocator);

    // Copy the test data into the allocated buffer to simulate having received it from
    // the X server
    @memcpy(connect_setup_stub.buf, &TEST_RECEIVED_CONNECT_SETUP_BUFFER);
    ...
    try testing.expectEqual(x.Visual.fromInt(1946), matching_visual_type_24.id);
    ...
    try testing.expectEqual(x.Visual.fromInt(124), matching_visual_type_32.id);
...
test "Parse received ConnectSetup message" {
    ...
    try testing.expectEqual(x.ResourceBase.fromInt(396361728), fixed.resource_id_base);
    ...
    try testing.expectEqual(x.Window.fromInt(1971), screens[0].root);
    try testing.expectEqual(x.ColorMap.fromInt(32), screens[0].colormap);
    ...
    try testing.expectEqual(x.Visual.fromInt(33), screens[0].root_visual);
    ...
    try testing.expectEqual(x.Window.fromInt(1972), screens[1].root);
            ...
            switch (screen_depth.depth) {
                1, 4 => {
                    for (visual_types) |visual_type| {
                        try testing.expectEqual(x.Visual.fromInt(0), visual_type.id);
                ...
                24 => {
                    try testing.expectEqual(x.Visual.fromInt(33), visual_types[0].id);
                    ...
                    try testing.expectEqual(x.Visual.fromInt(1683), visual_types[1].id);

                    try testing.expectEqual(x.Visual.fromInt(1946), visual_types[2].id);
                    try testing.expectEqual(x.VisualType.Class.direct_color, visual_types[2].class);
                },
                32 => {
                    try testing.expectEqual(x.Visual.fromInt(124), visual_types[0].id);
                    try testing.expectEqual(x.VisualType.Class.true_color, visual_types[0].class);
                    try testing.expectEqual(@as(u8, 8), visual_types[0].bits_per_rgb_value);
                    try testing.expectEqual(@as(u16, 256), visual_types[0].colormap_entries);
                    try testing.expectEqual(@as(u32, 0xff0000), visual_types[0].red_mask);
                    ...
                    try testing.expectEqual(x.Visual.fromInt(1947), visual_types[1].id);

                    try testing.expectEqual(x.Visual.fromInt(1969), visual_types[2].id);
                },
                else => unreachable,
            }
        }
